...
public class BrandSafetyTask {
    ...
    public void computeBrandSafety() {
        ...
        var bsTable = table(name("twitch", "channel_brand_safety"));

        Result<? extends Record> channels = dsl.selectDistinct(
                        field(name("twitch", "session", "twitch_id"), Long.class)
                )
                .from(sessionTable)
                .leftJoin(bsTable)
                .on(field(name("twitch", "session", "twitch_id"))
                        .eq(field(name("twitch", "channel_brand_safety", "twitch_id"))))
                .where(field(name("twitch", "channel_brand_safety", "twitch_id")).isNull()
                        .or(field(name("twitch", "channel_brand_safety", "updated_at"))
        ...
        for (var row : channels) {
            Long twitchId = row.get("twitch_id", Long.class);
            if (twitchId == null) continue;

            try {
                brandSafetyService.computeAndSave(twitchId);
            } catch (Exception e) {
                log.error("Failed to compute brand safety for {}: {}", twitchId, e.getMessage());
            }
        }
    }
}
